...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
         ...
         */
        proc->wait_state = 0;             // 等待状态：0
        proc->cptr = NULL;                // 子进程：NULL
        proc->yptr = NULL;                // 弟弟进程：NULL
        proc->optr = NULL;                // 哥哥进程：NULL
        // LAB6:2310432 (update LAB5 steps)
        /*
         ...
         */
        proc->rq = NULL;                  // 运行队列：NULL
        list_init(&(proc->run_link));     // 初始化运行队列链表项
        proc->time_slice = 0;             // 时间片：0
        proc->lab6_run_pool.left = NULL;  // 斜堆左子树：NULL
        proc->lab6_run_pool.right = NULL; // 斜堆右子树：NULL
        proc->lab6_run_pool.parent = NULL;// 斜堆父节点：NULL
        proc->lab6_stride = 0;            // stride值：0
        proc->lab6_priority = 0;          // 优先级：0
    }
    return proc;
}
...
load_icode(unsigned char *binary, size_t size)
{
    ...
    tf->status |= SSTATUS_SPIE;  // 用户模式开启中断
