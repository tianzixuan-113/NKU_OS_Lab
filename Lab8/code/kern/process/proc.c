    ...
    //LAB8 YOUR CODE : (update LAB4 steps)
       ...
       */   
        ...
        local_intr_save(intr_flag);
        {
            ...
            // LAB8: 刷新 TLB
            flush_tlb();
            // 切换页表 - 注意：pgdir 存储的是物理地址
            if (proc->pgdir != 0) {
                lsatp(proc->pgdir);
            } else {
                // 如果 pgdir 为 0，使用内核页目录
                lsatp(boot_pgdir_pa);
            }
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    if (current->wait_state != 0) {
        panic("do_fork: parent wait_state != 0");
    }
    ...
    proc->pid = get_pid();
    hash_proc(proc);
    set_links(proc);  // lab5新增：设置进程关系链接
...
load_icode(int fd, int argc, char **kargv)
{
    ...
    for (int i = 0; i < elf.e_phnum; i++) {
        ...
        // Set permission bits for RISC-V
        ...
        /* (3.5) call pgdir_alloc_page to allocate page for TEXT/DATA, 
        ...
        /* (3.6) call pgdir_alloc_page to allocate pages for BSS, memset zero in these pages */
        if (ph.p_memsz > ph.p_filesz) {
            uintptr_t start = ph.p_va + ph.p_filesz;
            uintptr_t end = ph.p_va + ph.p_memsz;
            uintptr_t la = ROUNDDOWN(start, PGSIZE);
            
            // Handle partial page at the beginning
            if (start < la + PGSIZE) {
                if ((page = pgdir_alloc_page(mm->pgdir, la, perm)) == NULL) {
                    goto bad_cleanup_mmap;
                }
    ...
    char *ustack_argv[EXEC_MAX_ARG_NUM];
    for (int i = 0; i < argc; i++) {
        size_t len = strlen(kargv[i]) + 1;
        stacktop -= len;
        memcpy((void *)stacktop, kargv[i], len);
        ustack_argv[i] = (char *)stacktop;
    }
    ...
    // Push argv pointers
    stacktop -= (argc + 1) * sizeof(char *);
    char **ustack_argvp = (char **)stacktop;
    for (int i = 0; i < argc; i++) {
        ustack_argvp[i] = ustack_argv[i];
    }
    ustack_argvp[argc] = NULL;
    ...
    *(int *)stacktop = argc;
    ...
    tf->status |= SSTATUS_SPIE;  // Enable interrupts in user mode
